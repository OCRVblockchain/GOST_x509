...
func createStoreContext(leaf *Certificate, opts *VerifyOptions) (*syscall.CertContext, error) {
	...
	leafCtx, err := syscall.CertCreateCertificateContext(syscall.gost509_ASN_ENCODING|syscall.PKCS_7_ASN_ENCODING, &leaf.Raw[0], uint32(len(leaf.Raw)))
	if err != nil {
		return nil, err
	}
	...
	if opts.Intermediates != nil {
		for i := 0; i < opts.Intermediates.len(); i++ {
			...
			ctx, err := syscall.CertCreateCertificateContext(syscall.gost509_ASN_ENCODING|syscall.PKCS_7_ASN_ENCODING, &intermediate.Raw[0], uint32(len(intermediate.Raw)))
			if err != nil {
				return nil, err
			}
...
func extractSimpleChain(simpleChain **syscall.CertSimpleChain, count int) (chain []*Certificate, err error) {
	if simpleChain == nil || count == 0 {
		return nil, errors.New("gost509: invalid simple chain")
	}
...
func verifyChain(c *Certificate, chainCtx *syscall.CertChainContext, opts *VerifyOptions) (chain []*Certificate, err error) {
	...
	if len(chain) == 0 {
		return nil, errors.New("gost509: internal error: system verifier returned an empty chain")
	}
