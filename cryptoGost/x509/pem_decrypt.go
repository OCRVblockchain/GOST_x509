...
var IncorrectPasswordError = errors.New("gost509: decryption password incorrect")
...
func DecryptPEMBlock(b *pem.Block, password []byte) ([]byte, error) {
	...
	if !ok {
		return nil, errors.New("gost509: no DEK-Info header in block")
	}

	mode, hexIV, ok := strings.Cut(dek, ",")
	if !ok {
		return nil, errors.New("gost509: malformed DEK-Info header")
	}

	ciph := cipherByName(mode)
	if ciph == nil {
		return nil, errors.New("gost509: unknown encryption mode")
	}
	...
	if len(iv) != ciph.blockSize {
		return nil, errors.New("gost509: incorrect IV size")
	}
	...
	if len(b.Bytes)%block.BlockSize() != 0 {
		return nil, errors.New("gost509: encrypted PEM data is not a multiple of the block size")
	}
	...
	if dlen == 0 || dlen%ciph.blockSize != 0 {
		return nil, errors.New("gost509: invalid padding")
	}
...
func EncryptPEMBlock(rand io.Reader, blockType string, data, password []byte, alg PEMCipher) (*pem.Block, error) {
	...
	if _, err := io.ReadFull(rand, iv); err != nil {
		return nil, errors.New("gost509: cannot generate IV: " + err.Error())
	}
