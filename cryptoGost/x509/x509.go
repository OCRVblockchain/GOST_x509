...
// Package gost509 parses X.509-encoded keys and certificates.
package gost509
...
func ParsePKIXPublicKey(derBytes []byte) (pub any, err error) {
	...
	if rest, err := asn1.Unmarshal(derBytes, &pki); err != nil {
		if _, err := asn1.Unmarshal(derBytes, &pkcs1PublicKey{}); err == nil {
			return nil, errors.New("gost509: failed to parse public key (use ParsePKCS1PublicKey instead for this key format)")
		}
		return nil, err
	} else if len(rest) != 0 {
		return nil, errors.New("gost509: trailing data after ASN.1 of public-key")
	}
	algo := getPublicKeyAlgorithmFromOID(pki.Algorithm.Algorithm)
	if algo == UnknownPublicKeyAlgorithm {
		return nil, errors.New("gost509: unknown public key algorithm")
	}
...
func marshalPublicKey(pub any) (publicKeyBytes []byte, publicKeyAlgorithm pkix.AlgorithmIdentifier, err error) {
	...
	case *ecdsa.PublicKey:
		...
		if !ok {
			return nil, pkix.AlgorithmIdentifier{}, errors.New("gost509: unsupported elliptic curve")
		}
	...
	case *gost3410.PublicKey:
		var key gost3410.PublicKeyInfo
		var paramset asn1.ObjectIdentifier
		var algorithm asn1.ObjectIdentifier

		switch pub.C {
		case gost3410.CurveIdtc26gost34102012256paramSetA:
			paramset = gost3410.OidTc26Gost34102012256ParamSetA
			algorithm = gost3410.OidTc26Gost34112012256

		case gost3410.CurveIdtc26gost341012512paramSetA:
			paramset = gost3410.OidTc26Gost34102012512ParamSetA
			algorithm = gost3410.OidTc26Gost34112012512
		default:
			return nil, pkix.AlgorithmIdentifier{}, errors.New("gost509: unknown elliptic curve")
		}

		key, err = gost3410.GenKey(pub, paramset, algorithm)
		if err != nil {
			return
		}
		return key.PublicKey.Bytes, key.Algorithm, nil
	default:
		return nil, pkix.AlgorithmIdentifier{}, fmt.Errorf("gost509: unsupported public key type: %T", pub)
	}
...
var (
	...
	oidSignatureGOST34102012WithSHA256 = asn1.ObjectIdentifier{1, 2, 643, 7, 1, 1, 3, 2}
	oidSignatureGOST34102012WithSHA512 = asn1.ObjectIdentifier{1, 2, 643, 7, 1, 1, 3, 3}
...
var signatureAlgorithmDetails = []struct {
	...
	{SHA256Gost, "SHA256-GOST", oidSignatureGOST34102012WithSHA256, Gost3010, crypto.SHA256},
	{SHA512Gost, "SHA512-GOST", oidSignatureGOST34102012WithSHA512, Gost3010, crypto.SHA512},
...
func getPublicKeyAlgorithmFromOID(oid asn1.ObjectIdentifier) PublicKeyAlgorithm {
	...
	case oid.Equal(oidPublicKeyEd25519):
		return Ed25519
	case oid.Equal(oidPublicKeyGost256), oid.Equal(oidPublicKeyGost512):
		return Gost3010
	}
...
var (
	...
	oidNamedCurveGOST512 = asn1.ObjectIdentifier{1, 3, 132, 0, 35}
)
...
var ErrUnsupportedAlgorithm = errors.New("gost509: cannot verify signature: algorithm unimplemented")
...
// "gost509sha1=1" in the GODEBUG environment variable. Note that this option will
...
func (e InsecureAlgorithmError) Error() string {
	...
	if SignatureAlgorithm(e) == SHA1WithRSA || SignatureAlgorithm(e) == ECDSAWithSHA1 {
		override = " (temporarily override with GODEBUG=gost509sha1=1)"
	}
	return fmt.Sprintf("gost509: cannot verify signature: insecure algorithm %v", SignatureAlgorithm(e)) + override
}
...
func (ConstraintViolationError) Error() string {
	return "gost509: invalid signature: parent certificate cannot sign this kind of certificate"
}
...
func signaturePublicKeyAlgoMismatchError(expectedPubKeyAlgo PublicKeyAlgorithm, pubKey any) error {
	return fmt.Errorf("gost509: signature algorithm specifies an %s public key, but have public key of type %T", expectedPubKeyAlgo.String(), pubKey)
}
...
func checkSignature(algo SignatureAlgorithm, signed, signature []byte, publicKey crypto.PublicKey) (err error) {
	...
	case *ecdsa.PublicKey:
		...
		if !ecdsa.VerifyASN1(pub, signed, signature) {
			return errors.New("gost509: ECDSA verification failure")
		}
		return
	case ed25519.PublicKey:
		if pubKeyAlgo != Ed25519 {
		...
		if !ed25519.Verify(pub, signed, signature) {
			return errors.New("gost509: Ed25519 verification failure")
		}
		return
	case *gost3410.PublicKey:
		if pubKeyAlgo != Gost3010 {
			return signaturePublicKeyAlgoMismatchError(pubKeyAlgo, pub)
		}

		p, er := pub.VerifyDigest(signed, signature)
		if er != nil {
			return er
		}

		if !p {
			return errors.New("gost509: GOST verification failure")
		}
		return
	}
	return ErrUnsupportedAlgorithm
...
func (h UnhandledCriticalExtension) Error() string {
	return "gost509: unhandled critical extension"
}
...
func isIA5String(s string) error {
	for _, r := range s {
		...
		if r > unicode.MaxASCII {
			return fmt.Errorf("gost509: %q cannot be encoded as an IA5String", s)
		}
	}
...
func marshalExtKeyUsage(extUsages []ExtKeyUsage, unknownUsages []asn1.ObjectIdentifier) (pkix.Extension, error) {
	...
	for i, u := range extUsages {
		if oid, ok := oidFromExtKeyUsage(u); ok {
			...
			return ext, errors.New("gost509: unknown extended key usage")
		}
	}
...
func signingParamsForPublicKey(pub any, requestedSigAlgo SignatureAlgorithm) (hashFunc crypto.Hash, sigAlgo pkix.AlgorithmIdentifier, err error) {
	...
	case *ecdsa.PublicKey:
		...
		default:
			err = errors.New("gost509: unknown elliptic curve")
		}
	...
	case *gost3410.PublicKey:
		...
		switch pub.C {
		case gost3410.CurveIdtc26gost34102012256paramSetA, gost3410.CurveIdtc26gost34102012256paramSetB,
			gost3410.CurveIdtc26gost34102012256paramSetC, gost3410.CurveIdtc26gost34102012256paramSetD:

			hashFunc = crypto.SHA256
			sigAlgo.Algorithm = oidSignatureGOST34102012WithSHA256
			sigAlgo.Parameters = asn1.RawValue{FullBytes: []byte{48, 52, 160, 15, 48, 13, 6, 9, 96, 134, 72, 1, 101, 3, 4, 2, 1, 5, 0, 161, 28, 48, 26, 6, 9, 42, 134, 72, 134, 247, 13, 1, 1, 8, 48, 13, 6, 9, 96, 134, 72, 1, 101, 3, 4, 2, 1, 5, 0, 162, 3, 2, 1, 32}}
		case gost3410.CurveIdtc26gost341012512paramSetA, gost3410.CurveIdtc26gost341012512paramSetB,
			gost3410.CurveIdtc26gost34102012512paramSetC:
			hashFunc = crypto.SHA512
			sigAlgo.Algorithm = oidSignatureGOST34102012WithSHA512
	...
	default:
		err = errors.New("gost509: only GOST3410, RSA, ECDSA and Ed25519 keys supported")
	}
	...
	for _, details := range signatureAlgorithmDetails {
		if details.algo == requestedSigAlgo {
			if details.pubKeyAlgo != pubType {
				err = errors.New("gost509: requested SignatureAlgorithm does not match private key type")
				return
			}
			sigAlgo.Algorithm, hashFunc = details.oid, details.hash
			if hashFunc == 0 && pubType != Ed25519 {
				err = errors.New("gost509: cannot sign with hash function requested")
				return
			}
	...
	if !found {
		err = errors.New("gost509: unknown SignatureAlgorithm")
	}
...
func CreateCertificate(rand io.Reader, template, parent *Certificate, pub, priv any) ([]byte, error) {
	...
	if !ok {
		return nil, errors.New("gost509: certificate private key does not implement crypto.Signer")
	}

	if template.SerialNumber == nil {
		return nil, errors.New("gost509: no SerialNumber given")
	}

	if template.BasicConstraintsValid && !template.IsCA && template.MaxPathLen != -1 && (template.MaxPathLen != 0 || template.MaxPathLenZero) {
		return nil, errors.New("gost509: only CAs are allowed to specify MaxPathLen")
	}
	...
	if privPub, ok := key.Public().(privateKey); !ok {
		return nil, errors.New("gost509: internal error: supported public key does not implement Equal")
	} else if parent.PublicKey != nil && !privPub.Equal(parent.PublicKey) {
		return nil, errors.New("gost509: provided PrivateKey doesn't match parent's PrivateKey")
	}
	...
	default:
		if err := checkSignature(sigAlg, c.Raw, signature, pub); err != nil {
			return nil, fmt.Errorf("gost509: signature over certificate returned by signer is invalid: %w", err)
		}
	}
...
func ParseDERCRL(derBytes []byte) (*pkix.CertificateList, error) {
	...
	if rest, err := asn1.Unmarshal(derBytes, certList); err != nil {
		...
		return nil, errors.New("gost509: trailing data after CRL")
	}
	return certList, nil
}
...
func newRawAttributes(attributes []pkix.AttributeTypeAndValueSET) ([]asn1.RawValue, error) {
	...
	if len(rest) != 0 {
		return nil, errors.New("gost509: failed to unmarshal raw CSR Attributes")
	}
...
func CreateCertificateRequest(rand io.Reader, template *CertificateRequest, priv any) (csr []byte, err error) {
	...
	if len(extensions) > 0 && !extensionsAppended {
		...
		if err != nil {
			return nil, errors.New("gost509: failed to serialise extensions attribute: " + err.Error())
		}
...
func parseCertificateRequest(in *certificateRequest) (*CertificateRequest, error) {
	...
	if rest, err := asn1.Unmarshal(in.TBSCSR.Subject.FullBytes, &subject); err != nil {
		...
		return nil, errors.New("gost509: trailing data after X.509 Subject")
	}
...
func CreateRevocationList(rand io.Reader, template *RevocationList, issuer *Certificate, priv crypto.Signer) ([]byte, error) {
	if template == nil {
		return nil, errors.New("gost509: template can not be nil")
	}
	if issuer == nil {
		return nil, errors.New("gost509: issuer can not be nil")
	}
	if (issuer.KeyUsage & KeyUsageCRLSign) == 0 {
		return nil, errors.New("gost509: issuer must have the crlSign key usage bit set")
	}
	if len(issuer.SubjectKeyId) == 0 {
		return nil, errors.New("gost509: issuer certificate doesn't contain a subject key identifier")
	}
	if template.NextUpdate.Before(template.ThisUpdate) {
		return nil, errors.New("gost509: template.ThisUpdate is after template.NextUpdate")
	}
	if template.Number == nil {
		return nil, errors.New("gost509: template contains nil Number field")
	}
