...
func isPrintable(b byte) bool {
	return 'a' <= b && b <= 'z' ||
		...
		// However, gost509 certificates with wildcard strings don't
...
func parseName(raw cryptobyte.String) (*pkix.RDNSequence, error) {
	if !raw.ReadASN1(&raw, cryptobyte_asn1.SEQUENCE) {
		return nil, errors.New("gost509: invalid RDNSequence")
	}
	...
	for !raw.Empty() {
		...
		for !set.Empty() {
			...
			if !set.ReadASN1(&atav, cryptobyte_asn1.SEQUENCE) {
				return nil, errors.New("gost509: invalid RDNSequence: invalid attribute")
			}
			var attr pkix.AttributeTypeAndValue
			if !atav.ReadASN1ObjectIdentifier(&attr.Type) {
				return nil, errors.New("gost509: invalid RDNSequence: invalid attribute type")
			}
			var rawValue cryptobyte.String
			var valueTag cryptobyte_asn1.Tag
			if !atav.ReadAnyASN1(&rawValue, &valueTag) {
				return nil, errors.New("gost509: invalid RDNSequence: invalid attribute value")
			}
			var err error
			attr.Value, err = parseASN1String(valueTag, rawValue)
			if err != nil {
				return nil, fmt.Errorf("gost509: invalid RDNSequence: invalid attribute value: %s", err)
			}
...
func parseAI(der cryptobyte.String) (pkix.AlgorithmIdentifier, error) {
	...
	if !der.ReadASN1ObjectIdentifier(&ai.Algorithm) {
		return ai, errors.New("gost509: malformed OID")
	}
	if der.Empty() {
		return ai, nil
	}
	...
	if !der.ReadAnyASN1Element(&params, &tag) {
		return ai, errors.New("gost509: malformed parameters")
	}
	...
	extract := func() (time.Time, error) {
		...
		case der.PeekASN1Tag(cryptobyte_asn1.UTCTime):
			...
			if !der.ReadASN1(&utc, cryptobyte_asn1.UTCTime) {
				return t, errors.New("gost509: malformed UTCTime")
			}
			s := string(utc)
		...
		case der.PeekASN1Tag(cryptobyte_asn1.GeneralizedTime):
			if !der.ReadASN1GeneralizedTime(&t) {
				return t, errors.New("gost509: malformed GeneralizedTime")
			}
		default:
			return t, errors.New("gost509: unsupported time format")
		}
		return t, nil
	}
...
func parseExtension(der cryptobyte.String) (pkix.Extension, error) {
	...
	if !der.ReadASN1ObjectIdentifier(&ext.Id) {
		return ext, errors.New("gost509: malformed extension OID field")
	}
	if der.PeekASN1Tag(cryptobyte_asn1.BOOLEAN) {
		if !der.ReadASN1Boolean(&ext.Critical) {
			return ext, errors.New("gost509: malformed extension critical field")
		}
	}
	var val cryptobyte.String
	if !der.ReadASN1(&val, cryptobyte_asn1.OCTET_STRING) {
		return ext, errors.New("gost509: malformed extension value field")
	}
	ext.Value = val
	return ext, nil
}
...
func parsePublicKey(algo PublicKeyAlgorithm, keyData *publicKeyInfo) (any, error) {
	...
	case RSA:
		...
		if !bytes.Equal(keyData.Algorithm.Parameters.FullBytes, asn1.NullBytes) {
			return nil, errors.New("gost509: RSA key missing NULL parameters")
		}

		p := &pkcs1PublicKey{N: new(big.Int)}
		if !der.ReadASN1(&der, cryptobyte_asn1.SEQUENCE) {
			return nil, errors.New("gost509: invalid RSA public key")
		}
		if !der.ReadASN1Integer(p.N) {
			return nil, errors.New("gost509: invalid RSA modulus")
		}
		if !der.ReadASN1Integer(&p.E) {
			return nil, errors.New("gost509: invalid RSA public exponent")
		}

		if p.N.Sign() <= 0 {
			return nil, errors.New("gost509: RSA modulus is not a positive number")
		}
		if p.E <= 0 {
			return nil, errors.New("gost509: RSA public exponent is not a positive number")
		}
	...
	case ECDSA:
		...
		if !paramsDer.ReadASN1ObjectIdentifier(namedCurveOID) {
			return nil, errors.New("gost509: invalid ECDSA parameters")
		}
		namedCurve := namedCurveFromOID(*namedCurveOID)
		if namedCurve == nil {
			return nil, errors.New("gost509: unsupported elliptic curve")
		}
		x, y := elliptic.Unmarshal(namedCurve, der)
		if x == nil {
			return nil, errors.New("gost509: failed to unmarshal elliptic curve point")
		}
		pub := &ecdsa.PublicKey{
			Curve: namedCurve,
			X:     x,
	...
	case Ed25519:
		...
		if len(keyData.Algorithm.Parameters.FullBytes) != 0 {
			return nil, errors.New("gost509: Ed25519 key encoded with illegal parameters")
		}
		if len(der) != ed25519.PublicKeySize {
			return nil, errors.New("gost509: wrong Ed25519 public key size")
		}
		return ed25519.PublicKey(der), nil
	case DSA:
		y := new(big.Int)
		if !der.ReadASN1Integer(y) {
			return nil, errors.New("gost509: invalid DSA public key")
		}
		pub := &dsa.PublicKey{
			Y: y,
		...
		if !paramsDer.ReadASN1(&paramsDer, cryptobyte_asn1.SEQUENCE) ||
			...
			return nil, errors.New("gost509: invalid DSA parameters")
		}
		if pub.Y.Sign() <= 0 || pub.Parameters.P.Sign() <= 0 ||
			pub.Parameters.Q.Sign() <= 0 || pub.Parameters.G.Sign() <= 0 {
			return nil, errors.New("gost509: zero or negative DSA parameter")
		}
		return pub, nil
	default:
		return nil, nil
...
func parseKeyUsageExtension(der cryptobyte.String) (KeyUsage, error) {
	...
	if !der.ReadASN1BitString(&usageBits) {
		return 0, errors.New("gost509: invalid key usage")
	}
...
func parseBasicConstraintsExtension(der cryptobyte.String) (bool, int, error) {
	var isCA bool
	if !der.ReadASN1(&der, cryptobyte_asn1.SEQUENCE) {
		return false, 0, errors.New("gost509: invalid basic constraints a")
	}
	if der.PeekASN1Tag(cryptobyte_asn1.BOOLEAN) {
		if !der.ReadASN1Boolean(&isCA) {
			return false, 0, errors.New("gost509: invalid basic constraints b")
		}
	}
	maxPathLen := -1
	if !der.Empty() && der.PeekASN1Tag(cryptobyte_asn1.INTEGER) {
		if !der.ReadASN1Integer(&maxPathLen) {
			return false, 0, errors.New("gost509: invalid basic constraints c")
		}
	}
...
func forEachSAN(der cryptobyte.String, callback func(tag int, data []byte) error) error {
	if !der.ReadASN1(&der, cryptobyte_asn1.SEQUENCE) {
		return errors.New("gost509: invalid subject alternative names")
	}
	for !der.Empty() {
		var san cryptobyte.String
		var tag cryptobyte_asn1.Tag
		if !der.ReadAnyASN1(&san, &tag) {
			return errors.New("gost509: invalid subject alternative name")
		}
	...
	err = forEachSAN(der, func(tag int, data []byte) error {
		...
		case nameTypeEmail:
			...
			if err := isIA5String(email); err != nil {
				return errors.New("gost509: SAN rfc822Name is malformed")
			}
			emailAddresses = append(emailAddresses, email)
		case nameTypeDNS:
			name := string(data)
			if err := isIA5String(name); err != nil {
				return errors.New("gost509: SAN dNSName is malformed")
			}
			dnsNames = append(dnsNames, string(name))
		case nameTypeURI:
			uriStr := string(data)
			if err := isIA5String(uriStr); err != nil {
				return errors.New("gost509: SAN uniformResourceIdentifier is malformed")
			}
			uri, err := url.Parse(uriStr)
			if err != nil {
				return fmt.Errorf("gost509: cannot parse URI %q: %s", uriStr, err)
			}
			if len(uri.Host) > 0 {
				if _, ok := domainToReverseLabels(uri.Host); !ok {
					return fmt.Errorf("gost509: cannot parse URI %q: invalid domain", uriStr)
				}
			}
		...
		case nameTypeIP:
			...
			default:
				return errors.New("gost509: cannot parse IP address of length " + strconv.Itoa(len(data)))
			}
		}
...
func parseExtKeyUsageExtension(der cryptobyte.String) ([]ExtKeyUsage, []asn1.ObjectIdentifier, error) {
	...
	if !der.ReadASN1(&der, cryptobyte_asn1.SEQUENCE) {
		return nil, nil, errors.New("gost509: invalid extended key usages")
	}
	for !der.Empty() {
...
func parseCertificatePoliciesExtension(der cryptobyte.String) ([]asn1.ObjectIdentifier, error) {
	...
	if !der.ReadASN1(&der, cryptobyte_asn1.SEQUENCE) {
		return nil, errors.New("gost509: invalid certificate policies")
	}
	for !der.Empty() {
...
func parseNameConstraintsExtension(out *Certificate, e pkix.Extension) (unhandled bool, err error) {
	...
	if !outer.ReadASN1(&toplevel, cryptobyte_asn1.SEQUENCE) ||
		...
		!toplevel.Empty() {
		return false, errors.New("gost509: invalid NameConstraints extension")
	}
	...
	if !havePermitted && !haveExcluded || len(permitted) == 0 && len(excluded) == 0 {
		...
		//   present”
		return false, errors.New("gost509: empty name constraints extension")
	}
	...
	getValues := func(subtrees cryptobyte.String) (dnsNames []string, ips []*net.IPNet, emails, uriDomains []string, err error) {
		for !subtrees.Empty() {
			...
			if !subtrees.ReadASN1(&seq, cryptobyte_asn1.SEQUENCE) ||
				...
				return nil, nil, nil, nil, fmt.Errorf("gost509: invalid NameConstraints extension")
			}
			...
			case dnsTag:
				...
				if err := isIA5String(domain); err != nil {
					return nil, nil, nil, nil, errors.New("gost509: invalid constraint value: " + err.Error())
				}
				...
				if _, ok := domainToReverseLabels(trimmedDomain); !ok {
					return nil, nil, nil, nil, fmt.Errorf("gost509: failed to parse dnsName constraint %q", domain)
				}
			...
			case ipTag:
				...
				default:
					return nil, nil, nil, nil, fmt.Errorf("gost509: IP constraint contained value of length %d", l)
				}

				if !isValidIPMask(mask) {
					return nil, nil, nil, nil, fmt.Errorf("gost509: IP constraint contained invalid mask %x", mask)
				}
			...
			case emailTag:
				...
				if strings.Contains(constraint, "@") {
					if _, ok := parseRFC2821Mailbox(constraint); !ok {
						return nil, nil, nil, nil, fmt.Errorf("gost509: failed to parse rfc822Name constraint %q", constraint)
					}
				} else {
			...
			case uriTag:
				...
				if net.ParseIP(domain) != nil {
					return nil, nil, nil, nil, fmt.Errorf("gost509: failed to parse URI constraint %q: cannot be IP address", domain)
				}
				...
				if _, ok := domainToReverseLabels(trimmedDomain); !ok {
					return nil, nil, nil, nil, fmt.Errorf("gost509: failed to parse URI constraint %q", domain)
				}
		...
		if len(e.Id) == 4 && e.Id[0] == 2 && e.Id[1] == 5 && e.Id[2] == 29 {
			...
			case 31:
				...
				if !val.ReadASN1(&val, cryptobyte_asn1.SEQUENCE) {
					return errors.New("gost509: invalid CRL distribution points")
				}
				for !val.Empty() {
					var dpDER cryptobyte.String
					if !val.ReadASN1(&dpDER, cryptobyte_asn1.SEQUENCE) {
						return errors.New("gost509: invalid CRL distribution point")
					}
			...
			case 35:
				...
				if !val.ReadASN1(&akid, cryptobyte_asn1.SEQUENCE) {
					return errors.New("gost509: invalid authority key identifier")
				}
			...
			case 14:
				...
				if !val.ReadASN1(&skid, cryptobyte_asn1.OCTET_STRING) {
					return errors.New("gost509: invalid subject key identifier")
				}
			...
			val := cryptobyte.String(e.Value)
			if !val.ReadASN1(&val, cryptobyte_asn1.SEQUENCE) {
				return errors.New("gost509: invalid authority info access")
			}
			for !val.Empty() {
...
func parseCertificate(der []byte) (*Certificate, error) {
	...
	if !input.ReadASN1Element(&input, cryptobyte_asn1.SEQUENCE) {
		return nil, errors.New("gost509: malformed certificate")
	}
	cert.Raw = input
	...
	if !input.ReadASN1Element(&tbs, cryptobyte_asn1.SEQUENCE) {
		return nil, errors.New("gost509: malformed tbs certificate")
	}
	...
	if !tbs.ReadOptionalASN1Integer(&cert.Version, cryptobyte_asn1.Tag(0).Constructed().ContextSpecific(), 0) {
		return nil, errors.New("gost509: malformed version")
	}
	if cert.Version < 0 {
		return nil, errors.New("gost509: malformed version")
	}
	...
	cert.Version++
	if cert.Version > 3 {
		return nil, errors.New("gost509: invalid version")
	}

	serial := new(big.Int)
	if !tbs.ReadASN1Integer(serial) {
		return nil, errors.New("gost509: malformed serial number")
	}
	...
	if !tbs.ReadASN1(&sigAISeq, cryptobyte_asn1.SEQUENCE) {
		return nil, errors.New("gost509: malformed signature algorithm identifier")
	}
	...
	if !input.ReadASN1(&outerSigAISeq, cryptobyte_asn1.SEQUENCE) {
		return nil, errors.New("gost509: malformed algorithm identifier")
	}
	if !bytes.Equal(outerSigAISeq, sigAISeq) {
		return nil, errors.New("gost509: inner and outer signature algorithm identifiers don't match")
	}
	...
	if !tbs.ReadASN1Element(&issuerSeq, cryptobyte_asn1.SEQUENCE) {
		return nil, errors.New("gost509: malformed issuer")
	}
	...
	if !tbs.ReadASN1(&validity, cryptobyte_asn1.SEQUENCE) {
		return nil, errors.New("gost509: malformed validity")
	}
	...
	if !tbs.ReadASN1Element(&spki, cryptobyte_asn1.SEQUENCE) {
		return nil, errors.New("gost509: malformed spki")
	}
	...
	if !spki.ReadASN1(&pkAISeq, cryptobyte_asn1.SEQUENCE) {
		return nil, errors.New("gost509: malformed public key algorithm identifier")
	}
	...
	if !spki.ReadASN1BitString(&spk) {
		return nil, errors.New("gost509: malformed subjectPublicKey")
	}
			...
			if present {
				...
				for !extensions.Empty() {
					...
					if !extensions.ReadASN1(&extension, cryptobyte_asn1.SEQUENCE) {
						return nil, errors.New("gost509: malformed extension")
					}
	...
	if !input.ReadASN1BitString(&signature) {
		return nil, errors.New("gost509: malformed signature")
	}
...
func ParseCertificate(der []byte) (*Certificate, error) {
	...
	if len(der) != len(cert.Raw) {
		return nil, errors.New("gost509: trailing data")
	}
	return cert, err
}
